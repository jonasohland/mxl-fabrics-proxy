...
type Assignment struct {
	...
	// Domain is a domain **name**, never a path. The agent resolves it against its own
	// mappings; the server has no business knowing where a domain lives on disk, and accepting
	// a path here would make the API a remote arbitrary-filesystem-write primitive (§7.2, §13).
