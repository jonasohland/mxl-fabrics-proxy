...
ContinuousFlowWriter::openSamples(std::uint64_t headIndex, std::size_t count) {
    ::mxlMutableWrappedMultiBufferSlice buffers{};
    mxl(::mxlFlowWriterOpenSamples, "failed to open samples", _writer,
        headIndex, count, &buffers);
    return Access{*this, buffers};
}
...
ContinuousFlowWriter::Access::Access(
    ContinuousFlowWriter& writer, ::mxlMutableWrappedMultiBufferSlice buffers)
    : _writer(writer),
      _buffers(buffers) {
}

::mxlMutableWrappedMultiBufferSlice const&
ContinuousFlowWriter::Access::buffers() const noexcept {
    return _buffers;
}
