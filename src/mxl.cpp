...
::mxlStatus Exception::status() const noexcept { return _status; }
...
Instance::Inner::~Inner() { ::mxlDestroyInstance(instance); }
...
void Instance::destroy(::mxlFlowWriter writer) const {
  mxl(::mxlReleaseFlowWriter, "failed to release flow reader", _inner->instance,
      writer);
}
void Instance::destroy(::mxlFlowReader reader) const {
  mxl(::mxlReleaseFlowReader, "failed to release flow reader", _inner->instance,
      reader);
}
...
::mxlInstance Instance::raw() noexcept { return _inner->instance; }
...
::mxlFlowReader DiscreteFlowReader::raw() { return _reader; }
...
std::uint64_t DiscreteFlowReader::getHeadIndex() const {
  ...
  mxl(::mxlFlowReaderGetRuntimeInfo, "failed to get flow info", _reader, &info);
...
DiscreteFlowReader::getGrainNonBlocking(std::uint64_t index) {
  ::mxlGrainInfo info{};
  std::uint8_t *payload = nullptr;
  mxl(::mxlFlowReaderGetGrainNonBlocking, "failed to get grain", _reader, index,
      &info, &payload);
  return {info, payload};
}
...
::mxlFlowReader ContinuousFlowReader::raw() noexcept { return _reader; }
...
::mxlFlowWriter DiscreteFlowWriter::raw() { return _writer; }
...
std::uint8_t *DiscreteFlowWriter::Access::data() noexcept { return _payload; }
...
::mxlFlowWriter ContinuousFlowWriter::raw() noexcept { return _writer; }
